load("ExtractedTetrapolarResponses.mat");
...
gx = [gx fliplr(gx)];
gy = [-0.05*ones([1, length(temps)]) 0.015*ones([1, length(temps)])];
...
box off
title("Ground Truth");
text(1.4, 60, "Pressed with probe", 'FontSize', 13);

subplot(3,1,3);
gy = [0.65*ones([1, length(temps)]) 1.0*ones([1, length(temps)])];
patch(gx, gy, phi, 'facealpha', 0.3, 'edgecolor', 'none');
colormap(gray);
clim([0 70]);
hold on
