...
void read_all_at_freq(int freq, double * rms_array, double * mag_array, double * phase_array, uint8_t num_elec) // added by david 11/12/23
{
    ...
    mux_write(CHIP_SEL_MUX_SRC, elec_to_mux[12], MUX_EN);
    mux_write(CHIP_SEL_MUX_SINK, elec_to_mux[13], MUX_EN);
    mux_write(CHIP_SEL_MUX_VP, elec_to_mux[14], MUX_EN);
    mux_write(CHIP_SEL_MUX_VN, elec_to_mux[15], MUX_EN);
    delayMicroseconds(150);
    read_signal_at_freq(freq, rms_array + num_meas, mag_array + num_meas, phase_array + num_meas, NULL, 0);

    mux_write(CHIP_SEL_MUX_SRC, elec_to_mux[4], MUX_EN);
    mux_write(CHIP_SEL_MUX_SINK, elec_to_mux[5], MUX_EN);
    mux_write(CHIP_SEL_MUX_VP, elec_to_mux[6], MUX_EN);
    mux_write(CHIP_SEL_MUX_VN, elec_to_mux[7], MUX_EN);
    delayMicroseconds(150);
    read_signal_at_freq(freq, rms_array + 1, mag_array + 1, phase_array + 1, NULL, 0);
    delayMicroseconds(150);

    mux_write(CHIP_SEL_MUX_SRC, elec_to_mux[8], MUX_EN);
    mux_write(CHIP_SEL_MUX_SINK, elec_to_mux[9], MUX_EN);
    mux_write(CHIP_SEL_MUX_VP, elec_to_mux[10], MUX_EN);
    mux_write(CHIP_SEL_MUX_VN, elec_to_mux[11], MUX_EN);
    delayMicroseconds(150);
    read_signal_at_freq(freq, rms_array + 2, mag_array + 2, phase_array + 2, NULL, 0);
    delayMicroseconds(150);
...
{   
  ...
  Serial.print(", ");
  Serial.print(signal_rms[1], 4);
  Serial.print(", ");
  Serial.println(signal_rms[2], 4);
  // for(i = 0; i < 1680; i++)
  // {       
  //   Serial.print(signal_rms[0], 4);
  //   Serial.print(", ");
